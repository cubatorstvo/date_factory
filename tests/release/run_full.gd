extends SceneTree
## Headless release FULL progression entry. Auto-quits with non-zero on failure.
## Artifacts: tests/release/artifacts/full_godot.log (via --log-file) + full_report.txt


const REPORT_RES := "res://tests/release/artifacts/full_report.txt"
...
func _run() -> void:
	...
	var suite_script: GDScript = load("res://tests/release/suites/full_progression_suite.gd") as GDScript
	if report_script == null or seed_script == null or helpers_script == null or suite_script == null:
		push_error("RELEASE_FULL_LOAD_FAIL")
		quit(2)
		return
	...
	if ok:
		print("RELEASE_FULL_PASS")
		quit(0)
	else:
		print("RELEASE_FULL_FAIL failed_step=%s errors=%s" % [report.failed_step, helpers.errors])
		quit(1)
