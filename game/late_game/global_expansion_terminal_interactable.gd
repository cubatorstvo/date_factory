class_name GlobalExpansionTerminalInteractable
...
func _ready() -> void:
	prompt_action = LateGameTypes.TERMINAL_PROMPT
	monitoring = false
	monitorable = true
	collision_layer = 4
	collision_mask = 0
...
func get_interaction_prompt(_player: Node) -> String:
	if not _is_unlocked():
		return LateGameTypes.TERMINAL_LOCKED_PROMPT
	return "[E] %s" % LateGameTypes.TERMINAL_PROMPT
