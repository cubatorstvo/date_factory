...
func _init() -> void:
	...
	_tutorials[ID_DATING_INTRO] = _tutorial(
		ID_DATING_INTRO,
		"Свидание",
		"С конкретной девушкой — одна обычная встреча в календарный день. Если слот уже использован: «Сегодня уже встречались. Следующая встреча: завтра.» Награда Риты — срочное такси $75 за дополнительную встречу в тот же день. На старте свидание идёт в квартире, в повседневном образе, с базовыми ходами и ходами характеристик. В каждом эпизоде выбери один ход. Известные предпочтения девушки окрашивают теги ходов. Текущий прогресс Комбо показан в интерфейсе свидания."
	)
	...
	_milestones[ID_STAGE_2] = _milestone(
		...
		PackedStringArray([
			"Этап города: 1 → 2",
			"Открыты Café и Leisure Center как места свиданий",
			"Открыт магазин одежды",
			"Текущая цель: Приоденься",
			"Марина — путь к первому наряду",
			"Открыт Ресторан как мировая локация",
	...
	_milestones[ID_STAGE_3] = _milestone(
		...
		PackedStringArray([
			...
			"Открыт Ресторан как место свиданий",
			"Открыты ходы одежды",
	...
	_milestones[ID_STAGE_4] = _milestone(
		...
		PackedStringArray([
			"Этап города: 2 → 3",
			"Квартира: до 12 / 12 предметов",
			"Полный ручной набор мест и одежды",
			"Появились новые девушки",
			"Появились новые соперники",
			"Новая цель: Учёная",
		])
