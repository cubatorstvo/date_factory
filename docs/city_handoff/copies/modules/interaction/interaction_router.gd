...
static func route(action_id: StringName, source: Node, _by: Node, payload: Dictionary = {}) -> void:
	...
	match str(action_id):
		...
		"take_gift":
			EventBus.toast("РџРѕР»РєРё СЃ РїРѕРґР°СЂРєР°РјРё СѓР±СЂР°РЅС‹ вЂ” РїРѕРєСѓРїР°Р№ РІ РіРѕСЂРѕРґРµ Рё РЅРѕСЃРё РІ РёРЅРІРµРЅС‚Р°СЂРµ", &"info")
		"wardrobe":
			_cycle_outfit()
		...
		"answer_doorbell":
			# Doorbell removed вЂ” tip player toward the table.
		...
		"fix_device":
			...
			EventBus.toast("РЈСЃС‚СЂРѕР№СЃС‚РІРѕ РїРµСЂРµР·Р°РїСѓС‰РµРЅРѕ", &"info")
		"crisis_fix":
			if Game.crises != null:
		...
		"toggle_auto":
			...
			EventBus.toast("РђРІС‚РѕРјР°С‚РёР·Р°С†РёСЏ СѓСЂРѕРІРµРЅСЊ %d" % Game.dating.automation_level, &"info")
		...
		"go_outside":
			...
			if not Game.city.outside_tip_shown:
				...
				var tip := "Р“РѕСЂРѕРґ: РєР°С„Рµ Two Hearts РїРѕ С‚С‘РїР»РѕР№ РІС‹РІРµСЃРєРµ."
				tip += " РџР°СЂРє Рё leisure (Р·Р°Р»/РєРЅРёР¶РЅС‹Р№/РєРёРЅРѕ/Р°СЂРєР°РґР°) РЅР° Р·Р°РїР°РґРµ." if park_open else " РџР°СЂРє вЂ” СЃРєРѕСЂРѕ."
				var agency_open: bool = Game.city != null and Game.city.is_district_unlocked(CityDistricts.AGENCY_ROW)
				tip += " Agency Row (С„РѕС‚Рѕ/Р±Р°СЂР±РµСЂ/РѕС„РёСЃ) РґР°Р»СЊС€Рµ РЅР° Р·Р°РїР°РґРµ." if agency_open else ""
		...
		"go_neighbor":
			...
			EventBus.toast("РљРІР°СЂС‚РёСЂР° СЃРѕСЃРµРґРєРё. РњРѕР¶РЅРѕ Р·Р°РіРѕРІРѕСЂРёС‚СЊ.", &"info")
		"neighbor_look":
			EventBus.toast("РЈСЋС‚РЅРѕ. РџР°С…РЅРµС‚ Р»Р°РјРёРЅР°С‚РѕРј Рё РЅР°РґРµР¶РґР°РјРё.", &"info")
		"city_rest":
			...
			if mult < 1.0:
				EventBus.toast("РЎРєР°РјРµР№РєР° СѓР¶Рµ РЅРµ С‚Р°Рє Р±РѕРґСЂРёС‚. (+%.0f%%)" % (mult * 100.0), &"info")
			else:
				EventBus.toast("РўС‹ РїРµСЂРµРІС‘Р» РґСѓС…. Р’РЅРёРјР°РЅРёРµ РІРѕСЃСЃС‚Р°РЅР°РІР»РёРІР°РµС‚СЃСЏ.", &"ok")
		"city_buy_gift":
		...
		"city_cafe_job":
			...
			EventBus.toast("РћРЅР»Р°Р№РЅ-С…Р°Р»С‚СѓСЂР°: РґРµРЅСЊРіРё РєР°РїР°СЋС‚.", &"money")
		"city_cafe_scroll":
			Game.economy.add(&"popularity", 0.4, &"scroll")
			Game.economy.add(&"attention", -0.2, &"scroll")
			EventBus.toast("РЎРєСЂРѕР»Р» СЂР°РґРё РїРѕРїСѓР»СЏСЂРЅРѕСЃС‚Рё. Р“Р»Р°Р·Р° РіРѕСЂСЏС‚.", &"info")
		"city_coffee":
			if Game.economy.try_spend({"money": 8.0}, &"coffee"):
				Game.economy.add(&"attention", 1.0, &"coffee")
				EventBus.toast("Р›Р°С‚С‚Рµ РєРѕСЂРїРѕСЂР°С‚РёРІРЅРѕР№ Р»СЋР±РІРё.", &"ok")
			else:
				EventBus.toast("Р”Р°Р¶Рµ РЅР° РєРѕС„Рµ РЅРµС‚.", &"warn")
		"city_workout":
			_open_gym_ui()
		"city_gym_pass":
			if Game.economy.try_spend({"money": 40.0}, &"gym"):
				Game.economy.max_attention += 0.5
				EventBus.toast("РђР±РѕРЅРµРјРµРЅС‚: РїРѕС‚РѕР»РѕРє РІРЅРёРјР°РЅРёСЏ РІС‹СЂРѕСЃ.", &"ok")
			else:
				EventBus.toast("Р—Р°Р» РЅРµ РґР»СЏ Р±РµРґРЅС‹С…... РїРѕРєР°.", &"warn")
		"city_park_fun":
			if Game.city != null and not Game.city.is_district_unlocked(CityDistricts.PARK_LEISURE):
				EventBus.toast("РџР°СЂРє РµС‰С‘ Р·Р°РєСЂС‹С‚", &"warn")
				return
			...
			if dmult < 1.0:
				EventBus.toast("РЈС‚РєРё СЃС‹С‚С‹. Р­С„С„РµРєС‚ СЃР»Р°Р±РµРµ (+%.0f%%)" % (dmult * 100.0), &"info")
			else:
				EventBus.toast("РЈС‚РєРё РѕРґРѕР±СЂСЏСЋС‚. +в­ђ", &"ok")
		"city_bar_drink":
			if Game.economy.try_spend({"money": 15.0}, &"bar"):
				Game.economy.add(&"scandal", 0.8, &"bar")
				Game.economy.add(&"popularity", 0.6, &"bar")
				EventBus.toast("РЁРѕС‚ Рё СЂРµРїСѓС‚Р°С†РёСЏ СЃ РїСЂРёРІРєСѓСЃРѕРј СЃРєР°РЅРґР°Р»Р°.", &"warn")
			else:
				EventBus.toast("Р‘Р°СЂРјРµРЅ СЃРјРѕС‚СЂРёС‚ РЅР° С‚РµР±СЏ РєР°Рє РЅР° РїСѓСЃС‚РѕР№ РєРѕС€РµР»С‘Рє.", &"warn")
		"city_karaoke":
			Game.economy.add(&"popularity", 1.0, &"karaoke")
			Game.economy.add(&"scandal", 0.5, &"karaoke")
			EventBus.toast("РўС‹ СЃРїРµР». Р“РѕСЂРѕРґ Р·Р°РїРѕРјРЅРёР». Рљ СЃРѕР¶Р°Р»РµРЅРёСЋ.", &"info")
		"city_bus_info":
			EventBus.toast("РњР°СЂС€СЂСѓС‚С‹: Р”РѕРј в†” РџР»РѕС‰Р°РґСЊ в†” РљРѕРЅРµС† СЃРІРµС‚Р° (СЃРєРѕСЂРѕ).", &"info")
		_:
			EventBus.toast("Р”РµР№СЃС‚РІРёРµ: %s" % str(action_id), &"info")
...
static func _city_buy_gift(gift_id: StringName, discount: float) -> void:
	...
	if not Game.economy.try_spend({"money": cost}, &"city_gift"):
		EventBus.toast("РќРµ С…РІР°С‚Р°РµС‚ РґРµРЅРµРі", &"warn")
		return
	Game.inventory._add_gift(gift_id, 1)
	EventBus.toast("РљСѓРїР»РµРЅРѕ: %s" % str(def.get("name", gift_id)), &"ok")
	Game.quests.complete("s1_money")


static func _open_gym_ui() -> void:
	if not DatePlaces.is_leisure_unlocked():
		EventBus.toast("Р—Р°Р» РѕС‚РєСЂРѕРµС‚СЃСЏ СЃ РїР°СЂРєРѕРІС‹Рј СЂР°Р№РѕРЅРѕРј", &"warn")
		return
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	...
	if gym != null and gym.has_method("open"):
		gym.call("open")
		return
	EventBus.toast("Р¤РёС‚РЅРµСЃ-Р·Р°Р» РЅРµРґРѕСЃС‚СѓРїРµРЅ", &"warn")


static func _open_photo_studio_ui() -> void:
	if not DatePlaces.is_agency_row_unlocked():
		EventBus.toast("Р¤РѕС‚РѕСЃС‚СѓРґРёСЏ РѕС‚РєСЂРѕРµС‚СЃСЏ СЃ СЂР°Р№РѕРЅРѕРј Р°РіРµРЅС‚СЃС‚РІР°", &"warn")
		return
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	...
	if ui != null and ui.has_method("open"):
		ui.call("open")
		return
	EventBus.toast("Р¤РѕС‚РѕСЃС‚СѓРґРёСЏ РЅРµРґРѕСЃС‚СѓРїРЅР°", &"warn")


static func _open_barber_ui() -> void:
	if not DatePlaces.is_agency_row_unlocked():
		EventBus.toast("Р‘Р°СЂР±РµСЂ РѕС‚РєСЂРѕРµС‚СЃСЏ СЃ СЂР°Р№РѕРЅРѕРј Р°РіРµРЅС‚СЃС‚РІР°", &"warn")
		return
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	...
	if ui != null and ui.has_method("open"):
		ui.call("open")
		return
	EventBus.toast("Р‘Р°СЂР±РµСЂ РЅРµРґРѕСЃС‚СѓРїРµРЅ", &"warn")
...
static func _open_district_gate_ui(district_id: String) -> void:
	if district_id == "":
		EventBus.toast("Р Р°Р№РѕРЅ РЅРµРёР·РІРµСЃС‚РµРЅ", &"warn")
		return
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	...
	if ui != null and ui.has_method("open"):
		...
		return
	EventBus.toast("РРЅС„РѕСЂРјР°С†РёСЏ Рѕ СЂР°Р№РѕРЅРµ РЅРµРґРѕСЃС‚СѓРїРЅР°", &"warn")


static func _open_agency_board_ui() -> void:
	if not DatePlaces.is_agency_row_unlocked():
		EventBus.toast("РћС„РёСЃ Р°РіРµРЅС‚СЃС‚РІР° РµС‰С‘ Р·Р°РєСЂС‹С‚", &"warn")
		return
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	...
	if ui != null and ui.has_method("open"):
		ui.call("open")
		return
	EventBus.toast("Р”РѕСЃРєР° СЂР°СЃРїРёСЃР°РЅРёСЏ РЅРµРґРѕСЃС‚СѓРїРЅР°", &"warn")


static func _open_bookstore() -> void:
	if not DatePlaces.is_leisure_unlocked():
		EventBus.toast("РљРЅРёР¶РЅС‹Р№ РѕС‚РєСЂРѕРµС‚СЃСЏ СЃ РїР°СЂРєРѕРј", &"warn")
		return
	...
	if Game.dating != null and not Game.dating.active_manual.is_empty() and Game.dating.has_method("note_bookstore_browse"):
		...
		# Soft hook near date booking вЂ” note for scheduled girl.
...
static func _open_arcade_minigame(from_date: bool, girl_id: String) -> void:
	if not DatePlaces.is_arcade_bookable() and not from_date:
		EventBus.toast("РђСЂРєР°РґР° РµС‰С‘ Р·Р°РєСЂС‹С‚Р°", &"warn")
		return
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	...
	if arcade == null:
		EventBus.toast("РђРІС‚РѕРјР°С‚ РЅРµРґРѕСЃС‚СѓРїРµРЅ", &"warn")
		return
	var gid := girl_id
...
static func _open_home_prep_menu(kind: String) -> void:
	...
	if options.is_empty():
		EventBus.toast("РџСѓСЃС‚Рѕ", &"warn")
		return
...
static func _open_shop_menu(shop_id: String) -> void:
	...
	if kind == "outfit":
		...
		items = ["homeware_next"]
	if items.is_empty():
		EventBus.toast("РњР°РіР°Р·РёРЅ РїСѓСЃС‚", &"warn")
		return
	var tree := Engine.get_main_loop() as SceneTree
	if tree != null:
	...
	for raw2 in items:
		if Game.inventory.can_buy_gift(StringName(str(raw2))):
			...
			Game.quests.complete("s1_money")
			return
	EventBus.toast("РќРµ С…РІР°С‚Р°РµС‚ РґРµРЅРµРі РЅР° С‚РѕРІР°СЂС‹", &"warn")


static func _take_food(food_id: String) -> void:
	if Game.inventory.carried_item != &"":
		EventBus.toast("РЎРЅР°С‡Р°Р»Р° РїРѕР»РѕР¶Рё С‚Рѕ, С‡С‚Рѕ РІ СЂСѓРєР°С…, РЅР° СЃС‚РѕР»", &"warn")
		return
	Game.inventory.carried_item = StringName("food:%s" % food_id)
	EventBus.carry_changed.emit(Game.inventory.carried_item)
	EventBus.toast("Р’Р·СЏР» РµРґСѓ вЂ” РѕС‚РЅРµСЃРё РЅР° СЃС‚РѕР»", &"ok")
...
static func _take_drink(drink_id: String) -> void:
	...
	EventBus.carry_changed.emit(Game.inventory.carried_item)
	EventBus.toast("Р’Р·СЏР» РЅР°РїРёС‚РѕРє вЂ” РѕС‚РЅРµСЃРё РЅР° СЃС‚РѕР»", &"ok")
...
static func _place_carried_on_table() -> void:
	...
	if carried.begins_with("food:"):
		...
		EventBus.toast("РќР° СЃС‚РѕР» РЅСѓР¶РЅР° РµРґР° РёР· С…РѕР»РѕРґРёР»СЊРЅРёРєР° РёР»Рё РЅР°РїРёС‚РѕРє РёР· СЏС‰РёРєРѕРІ", &"warn")
...
static func _try_start_home_date() -> void:
	if not Game.dating.has_scheduled_date() or not Game.dating.schedule.is_home():
		EventBus.toast("РЎРЅР°С‡Р°Р»Р° РЅР°Р·РЅР°С‡СЊ РґРѕРјР°С€РЅРµРµ СЃРІРёРґР°РЅРёРµ РІ С‚РµР»РµС„РѕРЅРµ", &"warn")
		return
	...
	if not Game.dating.schedule.is_table_ready():
		EventBus.toast("РџРѕР»РѕР¶Рё РЅР° СЃС‚РѕР» РµРґСѓ Рё РЅР°РїРёС‚РѕРє", &"warn")
		return
	...
	if until > DateSchedule.ARRIVE_EARLY_MIN:
		...
		EventBus.toast("РўС‹ СЃРµР» Р·Р° СЃС‚РѕР». Р–РґРё РёР»Рё РїСЂРѕРјРѕС‚Р°Р№ РІСЂРµРјСЏ.", &"info")
		return
	# Inside arrive window вЂ” start immediately (no doorbell).
...
static func wait_for_scheduled_time() -> void:
	...
	if not Game.dating.schedule.player_seated:
		EventBus.toast("РЎРЅР°С‡Р°Р»Р° СЃСЏРґСЊ Р·Р° СЃС‚РѕР»", &"info")
		return
...
static func stand_up_from_table() -> void:
	...
	EventBus.toast("РўС‹ РІСЃС‚Р°Р» РёР·-Р·Р° СЃС‚РѕР»Р°", &"info")
...
static func _try_start_no_prep_date(expected_place: String = "") -> void:
	if not Game.dating.has_scheduled_date() or not Game.dating.schedule.is_no_prep():
		EventBus.toast("РЎРЅР°С‡Р°Р»Р° РЅР°Р·РЅР°С‡СЊ СЃРІРёРґР°РЅРёРµ РІ С‚РµР»РµС„РѕРЅРµ (РєР°С„Рµ / РїР°СЂРє / СЂРµСЃС‚РѕСЂР°РЅ / РєРёРЅРѕ / Р°СЂРєР°РґР°)", &"warn")
		return
	var booked: String = Game.dating.schedule.place_id()
	if expected_place == "cafe" and booked != "cafe":
		EventBus.toast("РЈ РґРІРµСЂРё РєР°С„Рµ РЅСѓР¶РЅР° Р±СЂРѕРЅСЊ РЅР° РєР°С„Рµ", &"warn")
		return
	if expected_place == "park":
		if not DatePlaces.is_park_bookable():
			...
			return
		if booked != "park":
			EventBus.toast("Р—РґРµСЃСЊ РЅСѓР¶РЅР° Р±СЂРѕРЅСЊ РЅР° РїР°СЂРє", &"warn")
			return
	if expected_place == "restaurant" and booked == "restaurant" and not DatePlaces.is_restaurant_bookable():
		EventBus.toast("Р РµСЃС‚РѕСЂР°РЅ РµС‰С‘ Р·Р°РєСЂС‹С‚", &"warn")
		return
	if expected_place == "cinema":
		if not DatePlaces.is_cinema_bookable():
			EventBus.toast("РљРёРЅРѕ РµС‰С‘ Р·Р°РєСЂС‹С‚Рѕ", &"warn")
			return
		if booked != "cinema":
			EventBus.toast("Р—РґРµСЃСЊ РЅСѓР¶РЅР° Р±СЂРѕРЅСЊ РЅР° РєРёРЅРѕ", &"warn")
			return
	if expected_place == "arcade":
		if not DatePlaces.is_arcade_bookable():
			...
			return
		if booked != "arcade":
			EventBus.toast("Р—РґРµСЃСЊ РЅСѓР¶РЅР° Р±СЂРѕРЅСЊ РЅР° Р°СЂРєР°РґСѓ", &"warn")
			return
	...
	if expected_place == "restaurant" and booked != "restaurant":
		EventBus.toast("Р‘СЂРѕРЅСЊ РЅР° РґСЂСѓРіРѕРµ РјРµСЃС‚Рѕ: %s" % booked, &"warn")
		return
	var until: int = Game.dating.schedule.minutes_until_date()
	if until > DateSchedule.ARRIVE_EARLY_MIN and Game.time != null:
		var sched: Dictionary = Game.dating.schedule.scheduled
		EventBus.toast("РўС‹ СЃРµР» Рё Р¶РґС‘С€СЊ вЂ” РІСЂРµРјСЏ Р±РµР¶РёС‚ Рє СЃРІРёРґР°РЅРёСЋ", &"info")
		Game.time.skip_to_minutes(int(sched.get("day", 1)), int(sched.get("minutes", 0)))
	var target: String = Game.dating.schedule.target_id()
	var unique: bool = bool(Game.dating.schedule.scheduled.get("unique", ContentDB.girls.has(target)))
	Game.dating.schedule.player_seated = true
...
static func _prepare_default_date() -> bool:
	EventBus.toast("РќР°Р·РЅР°С‡Р°Р№ СЃРІРёРґР°РЅРёРµ РІ С‚РµР»РµС„РѕРЅРµ: РјРµСЃС‚Рѕ Рё РІСЂРµРјСЏ", &"info")
	return false
...
static func _start_date_with_transition(target: String, unique: bool, zone: StringName = &"restaurant") -> void:
	...
	var begin_date := func() -> void:
		...
		if not Game.dating.start_manual(target, unique):
			...
			## Keep stage-1 order: prepare в†’ city в†’ date (HUD/tip stay in sync).
			Game.quests.complete("s1_prepare")
			Game.quests.complete("s1_city")
...
static func _finale_station(station: String) -> void:
	match station:
		"messages":
			EventBus.toast("РЎС‚Р°РЅС†РёСЏ РїРµСЂРµРїРёСЃРєРё Р°РєС‚РёРІРёСЂРѕРІР°РЅР°", &"story")
		"outfit":
			...
			EventBus.toast("Р¤РёРЅР°Р»СЊРЅС‹Р№ РѕР±СЂР°Р· РіРѕС‚РѕРІ", &"story")
		"gift":
			if Game.inventory.gift_count(&"romance_cert") <= 0:
				Game.inventory._add_gift(&"romance_cert", 1)
			EventBus.toast("РђР±СЃСѓСЂРґРЅС‹Р№ РїРѕРґР°СЂРѕРє РїСЂРѕРёР·РІРµРґС‘РЅ", &"story")
		"delivery":
			EventBus.toast("Р”РѕСЃС‚Р°РІРєР° РЅР° РѕСЂР±РёС‚Сѓ РїРѕРґС‚РІРµСЂР¶РґРµРЅР°", &"story")
		"parallel":
			Game.dating.raise_automation(3)
			EventBus.toast("РџР°СЂР°Р»Р»РµР»СЊРЅС‹Рµ Р»РёРЅРёРё РІ РЅРѕСЂРјРµ", &"story")
		"scandal":
			Game.economy.set_value(&"scandal", maxf(0.0, Game.economy.get_value(&"scandal") - 5.0))
			EventBus.toast("РЎРєР°РЅРґР°Р» СЃС‚Р°Р±РёР»РёР·РёСЂРѕРІР°РЅ", &"story")
		"core":
			EventBus.toast("РЇРґСЂРѕ РјРµРіР°РјР°С€РёРЅС‹ РіСѓРґРёС‚", &"story")
			Game.facility.set_flag("finale_core_ready", true)
		_:
			EventBus.toast("РЎС‚Р°РЅС†РёСЏ %s" % station, &"story")
	Game.quests.complete("s6_mega")


static func _start_finale() -> void:
	if not Game.girls.unlock_algorithm_if_ready():
		EventBus.toast("Р¤РёРЅР°Р» РµС‰С‘ РЅРµ РіРѕС‚РѕРІ: РЅСѓР¶РЅС‹ РІСЃРµ РґРµРІСѓС€РєРё, РїРѕРїСѓР»СЏСЂРЅРѕСЃС‚СЊ, Р»РµРіРµРЅРґР° Рё РјРµРіР°РјР°С€РёРЅР°", &"warn")
		return
	if not Game.facility.has_flag("finale_core_ready"):
		EventBus.toast("РЎРЅР°С‡Р°Р»Р° Р°РєС‚РёРІРёСЂСѓР№ СЏРґСЂРѕ РјРµРіР°РјР°С€РёРЅС‹", &"warn")
		return
	...
	if not Game.facility.has_flag("finale_crisis_cleared"):
		if Game.crises != null and not Game.crises.is_active():
			...
			if Game.crises.begin_crisis("memory_desync") or Game.crises.begin_crisis("lights_out"):
				EventBus.toast("РџРµСЂРµРґ РђР»РіРѕСЂРёС‚РјРѕРј вЂ” РїРѕСЃР»РµРґРЅРёР№ РїСЂРѕСЃС‚СЂР°РЅСЃС‚РІРµРЅРЅС‹Р№ СЃР±РѕР№. РЈСЃС‚СЂР°РЅРё РЅРѕРіР°РјРё.", &"warn")
				return
			...
			EventBus.toast("РЎРЅР°С‡Р°Р»Р° СЃРЅРёРјРё Р°РєС‚РёРІРЅС‹Р№ РєСЂРёР·РёСЃ", &"warn")
			return
	if not Game.inventory.own_outfit(&"final_absurd"):
		Game.inventory.owned_outfits.append(&"final_absurd")
	Game.inventory.equip_outfit(&"final_absurd")
