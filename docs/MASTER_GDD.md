...
Opening, Core и Closing дают `+1` / `-1` по предпочтению девушки. Combo, одежда и подготовка квартиры входят в итоговый счёт. Combo: три последовательных успешных хода с тремя разными тегами дают `+1`, максимум один раз за свидание. Secondary больше нет.

Игрок получает три источника решений в каждом эпизоде: случайные BASE под ситуацию, UNLOCKABLE через прокачку героя и постоянные LOCAL ходы объектов выбранного места. LOCAL не зависят от Situation и не резервируют теги BASE. Единственный score от места: подготовленная квартира `0`, неподготовленная `-1`. Одежда — последовательный upgrade `casual 0 → business +1 → luxury +2` без предпочтений конкретной девушки. Player-facing текст использует глобальные Game Terms: жирный термин и tooltip.
