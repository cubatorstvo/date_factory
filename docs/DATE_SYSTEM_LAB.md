...
Autoload `AutomationService` — единственная gameplay-точка количества клонов, распределения, скорости производства, обработки прошедшего времени, покупки automation upgrades и накопленных автоматических свиданий. Подписывается на `TimeService.time_advanced`. Пока `unlocked == false`, производство равно 0.

Базовые скорости без upgrades:

```text
100 денег на одного эффективного рабочего клона за игровой час
0.1 automatic date на одного эффективного dating-клона за игровой час
```

Целая часть денежного производства идёт через `EconomyService.add_money`. Дробная часть хранится в `work_income_fraction`. Dating-клоны не запускают `DateSession` / `DateEngine` / `GirlState`: они накапливают агрегированный `completed_auto_dates`. Дробный прогресс — `dating_progress_fraction`. `advance_time(60)` и `60 × advance_time(1)` при одинаковом исходном состоянии дают одинаковые Money, `completed_auto_dates` и эквивалентные дробные остатки.
...
Сигналы сервиса: `automation_unlocked`, `clones_changed`, `allocation_changed`, `production_changed`, `upgrade_purchased`. `completed_auto_dates` готов к World Reach на следующем этапе; Stage 6 по-прежнему ждёт глобальную экспансию.
...
Production: `stage = 6`, `completion_requirement = null`. Пока requirement не задан, `can_complete_current_stage()` и `try_complete_current_stage()` возвращают `false`. `on_enter_effects` содержит только уже заданные канонические unlock'и этой главы; сейчас список пуст. После MAX отношений Президента: Stage 5 завершается → `stage = 6` → `on_enter_effects` Stage 6 → глава активна и ждёт World Reach / глобальную экспансию. Фабрика клонов к этому моменту уже открыта с Stage 5.

Когда у Stage 6 появится выполненный `completion_requirement` (будущий `WorldReachRequirement` или тестовый double), `try_complete_current_stage()` идёт по ветке последнего Stage: `finale_reached = true`, сигналы `stage_completed(6)` и `finale_reached()`, `StoryState.stage` остаётся `6`. Stage 7 не создаётся.
...
`can_complete_current_stage()`: `false`, если Finale уже достигнут, нет definition или `completion_requirement == null`; иначе `requirement.is_met()`. Для production Stage 6 с `null` — `false`.
...
Это статическое game data. Requirements не пишутся в save.
...
Для Stage 6 до World Reach: «STAGE 6» и «Incremental-прогрессия будет подключена следующим этапом.» После `stage_completed` Action Result / Event Log: «Stage завершён.» После `stage_changed`: «Начат Stage N.» и новая цель. `refresh()` после `stage_changed` обновляет World / Girls / Rivals / Dating / Progression / Factory — новый unlock из `on_enter_effects` уже виден. После входа в Stage 5 появляется раздел Фабрика: клоны, один slider Work ↔ Dating, hourly rates из `AutomationService`, `completed_auto_dates`, три upgrades и dev «+1 ИГРОВОЙ ЧАС» через `TimeService.advance_time(60)`.
