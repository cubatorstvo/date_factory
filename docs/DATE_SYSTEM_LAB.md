...
Seed: `work_basic` — «Работать», 60 минут. Source of truth оплаты — `WorkService.get_current_shift_income()` (`get_current_hourly_pay()` — alias): `100 * 2^career_rank` для рангов 0..3 → $100 / $200 / $400 / $800. Story Stage больше не повышает ставку. New Game: `career_progression_unlocked = false`, `career_rank = 0`, смена $100.

После MAX Начальницы шахты (`girl_mine_boss`) pipeline выдаёт `career_progression_unlock` («Карьерные связи»): `player.career_progression_unlocked = true`. Доход остаётся $100, пока игрок сам не возьмёт Career Advancement.

`WorkService.create_career_advancement_action()` — «Добиться повышения»: `time_cost_minutes = WORK_MINUTES` (60), `money_cost = 0`, без денежного эффекта. Requirements: work available today + unlock + Capital следующего ранга + rank < 3. Effects: `RecordWorkDayEffect` (тот же daily key `work`) + повышение `career_rank` на 1. Ранги 1 / 2 / 3 требуют Capital 1 / 3 / 5. Карьера опциональна: можно продолжать работать на текущем ранге.

GameSimulator всегда: `Доход за смену: $<current>`. После unlock: `Карьера: <rank> / 3`; при rank < 3 ещё `Следующий доход` и `Требование: Capital N`. Кнопка `Добиться повышения — 1 ч — $100 → $200` (фактические минуты смены, не 8 часов) если unlock + Capital + не max и work-слот свободен. При rank 3 — только доход $800 и `Карьера: 3 / 3`. До unlock — стартовая работа без career-контролов.
...
Первая смена без checkbox: usage 0→1, выплата 100% текущей карьерной ставки, 1 смена; затем доступно «Выйти на подработку». Дополнительная смена: usage 1→2, выплата 50% текущей ставки, 1 смена. Первая смена с checkbox: usage 0→2, выплата 150%, 2 смены. Rank 0: $100 / overtime extra $50 (combined $150). Rank 1: $200 / $100 extra ($300). Rank 2: $400 / $200 extra ($600). Rank 3: $800 / $400 extra ($1200). Оба UI-пути дают `usage_count = 2` за календарный день. Player-facing после обычной смены: «Сегодня уже работали.» После Оли и одной смены — кнопка подработки. Если в этот день уже работали или повышались, кнопки повышения нет.
...
required_filler_max_count: int
story_girl_required_rating: int
...
`StageDefinition` — canonical source для roster Stage 1–4 и Story Girl access numbers. Girl access (`MinStoryStageGirlRequirement`, `CurrentStageFillerMaxGirlRequirement`, `RatingGirlRequirement` для сюжетных девушек) и Rival access читают эти поля из каталога, а не дублируют их как независимые константы. City Stage по-прежнему только physical world expansion. Полный roster — [`STORY_STAGE_PROGRESSION.md`](STORY_STAGE_PROGRESSION.md). Количественный анализ pacing — [`MONTE_CARLO_PROGRESSION_LAB.md`](MONTE_CARLO_PROGRESSION_LAB.md).
...
|---|---|---|---|---|
| `RatingGirlRequirement` | `required_rating` (story girls: `StageDefinition.story_girl_required_rating`) | `RatingService.get_rating() >= required` | `"Рейтинг"` | `"<current> / <required>"` |
...
| `MinStoryStageGirlRequirement` | `minimum_story_stage` (resolved from `StageDefinition`) | `StageService.get_current_stage() >= assigned Story Stage` | `"Этап сюжета"` | `"Stage <current> / <required>"` |
| `CurrentStageFillerMaxGirlRequirement` | `story_stage`, `required_count` (story girls: `StageDefinition.stage` / `required_filler_max_count`) | число filler этого Stage с relationship MAX `>= required_count` | `"Девушки этапа"` | `"<current> / <required>"` |
...
|---|---|---|---|---|
| `alina` | Алина | `city_center` | `MinStoryStageGirlRequirement(1)` | `[]` |
| `vika` | Вика | `cafe` | `MinStoryStageGirlRequirement(1)` | `[]` |
| `dasha` | Даша | `cafe` | `MinStoryStageGirlRequirement(1)` | `[]` |
| `girl_actress` | Актриса | `city_center` | `MinStoryStageGirlRequirement(1)`, `CurrentStageFillerMaxGirlRequirement(1, 2)`, `RatingGirlRequirement(2)` | `RivalDefeatedGirlRequirement(rival_boris)` |
| `marina` | Марина | `clothing_store` | `MinStoryStageGirlRequirement(2)` | `[]` |
| `katya` | Катя | `furniture_store` | `MinStoryStageGirlRequirement(2)` | `OutfitAboveCasualGirlRequirement` |
| `lera` | Лера | `cafe` | `MinStoryStageGirlRequirement(2)` | `OutfitAboveCasualGirlRequirement` |
| `kira` | Кира | `cafe` | `MinStoryStageGirlRequirement(3)` | `OutfitAboveCasualGirlRequirement` |
| `olya` | Оля | `restaurant` | `MinStoryStageGirlRequirement(3)` | `OutfitAboveCasualGirlRequirement` |
| `girl_mine_boss` | Начальница шахты | `restaurant` | `MinStoryStageGirlRequirement(2)`, `CurrentStageFillerMaxGirlRequirement(2, 2)`, `RatingGirlRequirement(5)` | `RivalDefeatedGirlRequirement(rival_foreman)`, `OutfitAboveCasualGirlRequirement` |
| `girl_magazine_editor` | Редактор журнала | `cafe` | `MinStoryStageGirlRequirement(3)`, `CurrentStageFillerMaxGirlRequirement(3, 2)`, `RatingGirlRequirement(8)` | `RivalDefeatedGirlRequirement(rival_columnist)`, `OutfitAboveCasualGirlRequirement` |
| `sonya` | Соня | `city_center` | `MinStoryStageGirlRequirement(3)` | `OutfitAboveCasualGirlRequirement` |
| `nika` | Ника | `cafe` | `MinStoryStageGirlRequirement(4)` | `OutfitAboveCasualGirlRequirement` |
| `rita` | Рита | `restaurant` | `MinStoryStageGirlRequirement(4)` | `OutfitAboveCasualGirlRequirement` |
| `eva` | Ева | `restaurant` | `MinStoryStageGirlRequirement(4)` | `OutfitAboveCasualGirlRequirement` |
| `girl_scientist` | Учёная | `city_center` | `MinStoryStageGirlRequirement(4)`, `CurrentStageFillerMaxGirlRequirement(4, 2)`, `RatingGirlRequirement(11)` | `RivalDefeatedGirlRequirement(rival_academic)`, `OutfitAboveCasualGirlRequirement` |
| `girl_president` | Президент | `restaurant` | `MinStoryStageGirlRequirement(5)`, `RatingGirlRequirement(12)` | `RivalDefeatedGirlRequirement(rival_minister)`, `OutfitAboveCasualGirlRequirement` |

Filler доступны по Story Stage без соперника. Сюжетные: 2 of 3 current-stage filler MAX + Rating → знакомство → связанный Rival → победа → свидание → MAX → следующий Stage. Каноническая связь `RivalDefinition.linked_girl_id`; сервисы не хардкодят пары. `RivalsService.get_rivals_at_current_location` показывает сюжетного соперника только если `GirlsService.is_discovered(linked_girl_id)` и ordinary/story rivals при assigned `StageDefinition.stage <= Story Stage` (`RivalDefinition.minimum_story_stage` — fallback, если rival нет в roster). Stage 1 даёт три параллельных filler-линии (Алина, Вика, Даша); две завершённые открывают Actress (Rating 2). После Actress City Stage 2, Clothing Store, Café/Leisure DateVenues и текущая цель «Приоденься». Марина — Stage 2 Casual exception и optional path к первому Outfit. Остальные Stage 2+ girls требуют Outfit выше Casual; отказ: «Для этого свидания нужен образ интереснее повседневного.» Две из Stage 2 filler открывают Mine Boss (Rating 5); после неё unlock Career Progression, ставка смены остаётся $100. Две из Stage 3 filler (Кира, Оля, Соня) открывают Editor (Rating 8). После Editor City Stage 3 и 12 / 12 Apartment cap. Две из Stage 4 filler (Ника, Рита, Ева) открывают Scientist (Rating 11); Scientist MAX даёт Rating 12 и открывает President. Filler не завершают Stage. Roster: [`STORY_STAGE_PROGRESSION.md`](STORY_STAGE_PROGRESSION.md).
...
MAX Начальницы шахты (сюжетная, не filler) идёт через тот же `grant_filler_reward_for_girl`: `career_progression_unlock` / «Карьерные связи». Игрок получает доступ к Career Advancement; ставка смены сразу не растёт.
...
`discover_rival`: при первом открытии `discovered = true`, сигнал `rival_discovered`, `true`; повтор — состояние прежнее, `false`. `defeat_rival`: `discovered = true` и `defeated = true`; при первой победе сигнал `rival_defeated` и `true`; повтор — `false`, сигнал не повторяется. `get_rivals_at_current_location` читает `WorldService.get_current_location_id()` и текущий Story Stage, возвращает `RivalDefinition` с тем же `location_id` и assigned `StageDefinition.stage <= Story Stage` (`RivalDefinition.minimum_story_stage` — fallback вне roster), у которых `linked_girl_id` пустой или `GirlsService.is_discovered(linked_girl_id)`. До знакомства со связанной девушкой сюжетный соперник не попадает в список. Этот контракт общий для 2D GameSimulator и будущего 3D NPC (`rival_id` → те же `GameAction`).
...
Начальница шахты `girl_mine_boss`: `restaurant`, Stage 2 + Rating 5, соперник `rival_foreman`. Trait Любит рестораны. `relationship_max = 10`. Начально известных Tags нет. Date eligibility: Outfit выше Casual. Мировая локация ресторана открыта с Stage 2; DateVenue Restaurant — с Stage 3. После MAX — награда `career_progression_unlock` («Карьерные связи»): открывается Career Progression, ставка смены остаётся $100 до Career Advancement.
...
Разделы: СВИДАНИЕ, ДЕВУШКИ, СЛОЖНОСТЬ ДЕВУШЕК, ТЕГИ, БАЗОВЫЕ ХОДЫ, ХОДЫ ХАРАКТЕРИСТИК, ХОДЫ ОДЕЖДЫ, СИТУАЦИИ, МЕСТА, ЛОКАЛЬНЫЕ ОБЪЕКТЫ, ЛОКАЛЬНЫЕ ХОДЫ, НАРЯДЫ, ХАРАКТЕРИСТИКИ, ПРАВИЛА СВИДАНИЯ, БАЛАНС, MONTE CARLO LAB, ТЕСТОВОЕ СОСТОЯНИЕ, ВАЛИДАЦИЯ.
...
«MONTE CARLO LAB»: отдельный Developer Room panel (`ProgressionLabPanel`). Заголовок панели — Monte Carlo Progression Lab. Моделирует популяцию игроков Stage 1–4 через immutable StagePlan и production actions на изолированном snapshot `GameState`. Не расширяет `GameSimulator`. Полная спецификация — [`MONTE_CARLO_PROGRESSION_LAB.md`](MONTE_CARLO_PROGRESSION_LAB.md).
